...
mongodb.MongoClient.connect(uri, (err, client) => {
  ...
  app.route('/search/:theRequest').get((req, res) => {
    ...
    bookCollection.find({averageRating:{$gte:minStars}}).sort({[sort]:sortOrder}).toArray((err, docs) => {  //toArray
      if (err) {
        // if an error happens
...
mongodb.MongoClient.connect(uri, (err, client) => {
  ...
  app.route('/search/:theRequest').get((req, res) => {
    ...
    if(req.params.theRequest === "ALL"){
      ...
      console.log(req.params.theRequest);
      query = {"_id": mongoose.BSONType.ObjectId("605934fbfec9d88e9bd7a696")};
    }
    
    bookCollection.find(query).toArray((err, docs) => {  //toArray
      if (err) {
        // if an error happens
        res.send("Error in GET req.");
      } else {
  ...
  app.route('/book/:theRequest').get((req, res) => {
    ...
    bookCollection.find({"id": req.params.theRequest}).toArray((err, docs) => {  //toArray
      if (err) {
        // if an error happens
        res.send("Error in GET req.");
      } else {
  ...
  app.route('/author/:theRequest').get((req, res) => {
    ...
    authorCollection.find({"name": req.params.theRequest}).toArray((err, docs) => {  //toArray
      if (err) {
        // if an error happens
        res.send("Error in GET req.");
      } else {
...
mongodb.MongoClient.connect(uri, (err, client) => {
  // connect to your specific collection (a.k.a database) that you specified at the end of your URI (/database)
  const collection = client.db("BackUp").collection("BooksPart2");
